#Noesis python importer - Skate 1, 2 and 3 files - By Beedy, GHFear and tuukkas - Version 4.7.0
...
def registerNoesisTypes():
                ...
                noesis.setHandlerTypeCheck(handle, CheckModelTypeX360)
                noesis.setHandlerLoadModel(handle, noepyLoadModelX360)

                handle = noesis.register("EA Skate (1,2,3) 3D Sim (X360) " + VERSION, ".rx2; .dat")
                noesis.setHandlerTypeCheck(handle, CheckSimTypeX360)
                noesis.setHandlerLoadModel(handle, noepyLoadSim)

                handle = noesis.register("EA Skate (1,2,3) Textures (X360) " + VERSION, ".rx2; .dat")
                noesis.setHandlerTypeCheck(handle, CheckTextureTypeX360)
                noesis.setHandlerLoadRGBA(handle, noepyLoadRGBAXbox)

                handle = noesis.register("EA Skate (1,2,3) 3D Models (PS3) " + VERSION, ".psg; .dat; .rps3")
                noesis.setHandlerTypeCheck(handle, CheckModelTypePS3)
                noesis.setHandlerLoadModel(handle, noepyLoadModelPS3)

                handle = noesis.register("EA Skate (1,2,3) 3D Sim (PS3) " + VERSION, ".psg; .dat; .rps3")
                noesis.setHandlerTypeCheck(handle, CheckSimTypePS3)
                noesis.setHandlerLoadModel(handle, noepyLoadSim)

                handle = noesis.register("EA Skate (1,2,3) Textures (PS3) " + VERSION, ".psg; .dat; .rps3; .rpsgl")
...
def CheckModelTypePS3(data):
        ...
        if psgTypeID == b'\x00\x00\x00\x10':
                print("PSG TYPE: EA SKATE MESH")
                return 1
...
def CheckTextureTypePS3(data):
        ...
        if psgTypeID == b'\x00\x00\x00\x80':
...
#load the xbox model
def noepyLoadModelX360(data, mdlList):
         print("\n")
         print(">>>>>>>>>>>>START OF FILE<<<<<<<<<<<<<")
         print("\n")
         VIndexOffset = []
...
#load the ps3 model
def noepyLoadModelPS3(data, mdlList):
         print("\n")
         print(">>>>>>>>>>>>START OF FILE<<<<<<<<<<<<<")
         print("\n")
         VIndexOffset = []
...
#Read Mesh info
         for i in range(0, meshCount):
                                ...
                                foundUV2 = 0
                                foundUV3 = 0
                                for j in range(0, info[3]):
                                                type = bs.readBytes(8)
                                                position = int.from_bytes(type[2:4], byteorder='big')

                                                #Vertex Type

                                                if type[:2] == b'\x02\x03' and type[6:8] == b'\x00\x01':
                                                                VertType = 1
                                                                #FLOAT
                                                                
                                                elif type[:2] == b'\x03\x04' and type[6:8] == b'\x00\x01':
                                                                VertType = 2
                                                                #HALFFLOAT
                                                                
                                                elif type == b'\x00\x1A\x23\xA6\x00\x00\x00\x01': #find for ps3
                                                                VertType = 3
                                                                #USHORT
                                                                
                                                elif type[:2] == b'\x01\x04' and type[6:8] == b'\x00\x01':
                                                                VertType = 4
                                                                #SHORT

                                                #UV Position 1

                                                if type == b'\x00\x2C\x23\xA5\x00\x05\x00\x06': #find for ps3
                                                                UVpos.append(position)
                                                                UVtype.append(1)
                                                                foundUV1 = 1
                                                                #FLOAT UV1
     
                                                elif type[:2] == b'\x03\x04' and type[6:8] == b'\x08\x01':
                                                                UVpos.append(position)
                                                                UVtype.append(2)
                                                                foundUV1 = 1
                                                                #HALFFLOAT UV1

                                                elif type[:2] == b'\x03\x02' and type[6:8] == b'\x08\x01':
                                                                UVpos.append(position)
                                                                UVtype.append(3)
                                                                foundUV1 = 1
                                                                #HALFFLOAT UV1
                                                                
                                                elif type[:2] == b'\x01\x02' and type[6:8] == b'\x08\x01':
                                                                UVpos.append(position)
                                                                UVtype.append(4)
                                                                foundUV1 = 1
                                                                #SHORT UV1
                                                                
                                                elif type == b'\x00\x2C\x20\x59\x00\x05\x00\x06': #find for ps3
                                                                UVpos.append(position)
                                                                UVtype.append(5)
                                                                foundUV1 = 1
                                                                #USHORT UV1
                                                ...
                                                if type[:2] == b'\x01\x02' and type[6:8] == b'\x09\x01':
                                                                UV2pos.append(position)
                                                                UV2type.append(1)
                                                                foundUV2 = 1
                                                                #SHORT UV2
                                                else:
                                                                if j == (info[3] - 1) and foundUV2 == 0:
                                                                        UV2pos.append(position)
                                                                        UV2type.append(0)

                                                #UV Position 3

                                                if type == b'\x00\x2C\x23\x5F\x00\x05\x02\x08': #find for ps3
                                                                UV3pos.append(position)
                                                                UV3type.append(1)
                                                                foundUV3 = 1
                                                                #HALFFLOAT UV3
                                                else:
                                                                if j == (info[3] - 1) and foundUV3 == 0:
                                                                        UV3pos.append(position)
                                                                        UV3type.append(0)
         ...
         for i in range(0, meshCount):
                                ...
                                bs.seek(FInfo[i][0] + 8, NOESEEK_ABS) #Seek to faces info  
                                FCount = bs.read(">i")
